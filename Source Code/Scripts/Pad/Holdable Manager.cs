...
namespace HoldablePad
{
    public class Manager
    {
        ...
        public static void Load()
        {
            ...
            leftHandObject.name = "LeftHoldableStorage";
            rightHandObject.name = "RightHoldableStorage";
            ...
            GameObject handhelds = Main.hahahah;
            ...
            for (int i = 0; i < handhelds.transform.childCount; i++)
            {
                ...
                if (handheldObject.transform.Find("UsedBulletGameObject") != null)
                {
                    handheldObject.SetActive(true);
                    handheldObject.AddComponent<Gun>();
                    handheldObject.GetComponent<Gun>().isLeftHand = isLeft;
                }
                else
                {
        ...
        public static void IsEquipped(string theName)
        {
            ...
            for (int i = 0; i < leftHandObject.transform.childCount; i++)
            {
                if (theName == leftHandObject.transform.GetChild(i).gameObject.name)
                {
                    ...
                    {
                        Page.EQUIP = true;
                        Page.current = leftHandObject.transform.GetChild(i).gameObject.name;
                    }
                }
            }
            ...
            for (int i = 0; i < rightHandObject.transform.childCount; i++)
            {
                if (theName == rightHandObject.transform.GetChild(i).gameObject.name)
                {
                    ...
                    {
                        Page.EQUIP = true;
                        Page.current = rightHandObject.transform.GetChild(i).gameObject.name;
                    }
                }
            }
        }
